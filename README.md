...
<details>
  ...
  + 배경<br>
    : 문제 인식<br>
      초기 구현에서는 성공 시 "success" 문자열을, 실패 시 에러 메시지를 반환하는 비일관적인 구조로 API를 설계했습니다.<br>
      클라이언트는 `result !== "success"` 같은  문자열 비교로 성공 여부를 판단해야 했고, 성공 시 추가 데이터(생성된 리뷰 ID 등)를 전달할 방법이 없었습니다.<br>
      또한 서버 측 입력값 검증이 없어 잘못된 데이터가 DB에 저장될 위험이 있었습니다.
      - **문자열 의존**: `result !== "success"` 같은 하드코딩된 문자열 비교
      - **입력값 검증 부재**: 입력값 검증이 서버 측에 없어 잘못된 데이터 저장 위험이 존재했고. 또한 빈 리뷰, 500자 초과 등 잘못된 데이터가 DB에 저장될 위험이 존재했습니다.
